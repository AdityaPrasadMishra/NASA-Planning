...
class Problem:
    ...
    def __init__(self, robotModelFile, humanModelFile, robotProblemFile, domainTemplate,
        ...
        self.robotProblemFile = robotProblemFile
        ...
        orig_dom_file = '/tmp/orig_dom.pddl'

        original_prob_map = read_state_from_domain_file(robotModelFile,robotProblemFile)
        ...
        if not robotPlanFile:
            ...
            with open(robotPlanFile, 'r') as plan_file:
                ...
                self.plan = temp #[:-1]
                self.cost = len(temp) #int(temp[-1].split(' ')[3].strip())
        ...
        #print "Explaining for plans",self.plan
        #print "Symm diff size", len(set(self.human_state) - set(self.robot_state)), len(set(self.robot_state) - set(self.human_state))
        #exit(0)
    ...
    def orig_isGoal(self, state):
        temp_domain = '/tmp/test_domain.pddl'
        temp_problem = self.robotProblemFile 
        write_domain_file_from_state(state, self.domainTemplate, temp_domain)
        feasibility_flag = validate_plan(temp_domain, self.robotProblemFile, self.groundedRobotPlanFile)
    ...
    def ordinary_successors(self, node):
        ...
        for item in del_set:
            ...
            listOfSuccessors.append([list(new_state), "DEL-"+item])
