...
def read_state_from_domain_file(domainFileName, problemFileName):
    ...
    for act_name in domain_map["domain"]["actions"]:
        
        #act_name = list(act.keys())[0]
        #print (act,act_name,act[act_name][0])
        for part_name in domain_map["domain"]["actions"][act_name]:
            #part_name = list(act_parts.keys())[0]
            if part_name == "parameters":
                current_state.add(act_name+"-has-parameters-"+" ".join(domain_map["domain"]["actions"][act_name][part_name]))
            elif part_name == "precondition":
                for ty in domain_map["domain"]["actions"][act_name][part_name]:
                    if ty in precondition_types_simple:
                        pos_preconditions = domain_map["domain"]["actions"][act_name][part_name][ty]
                ...
                for ty in domain_map["domain"]["actions"][act_name][part_name]:
                    ...
                    if ty in effect_types_simple:
                        pos_effects = domain_map["domain"]["actions"][act_name][part_name][ty]
