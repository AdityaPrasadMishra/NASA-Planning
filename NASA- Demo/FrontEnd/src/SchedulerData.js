...
export default class SchedulerData {
    ...
    updateEventsStart(events,newStart) {
        events.forEach((event)=>{
            this.updateEventStart(event, newStart);          
        })    
    }
    ...
    updateEventsEnd(events,newEnd) {
        events.forEach((event)=>{
            this.updateEventEnd(event, newEnd);          
        })    
    }
    ...
    _validateTasks(tasks){
        if(Object.prototype.toString.call(tasks) !== "[object Array]") {
            throw new Error('Tasks should be Array object');
        }

        tasks.forEach((e, index) => {
            if(e == undefined) {
                console.error(`Task undefined: ${index}`);
                throw new Error(`Task undefined: ${index}`);
            }
            // if(e.id == undefined || e.resourceId == undefined || e.title == undefined || e.start == undefined || e.end == undefined)
            // {
            //     console.error('Task property missed', index, e);
            //     throw new Error(`Event property undefined: ${index}`);
            // }
        });
    }
